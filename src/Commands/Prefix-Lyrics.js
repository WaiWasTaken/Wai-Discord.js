...
module.exports = {
    ...
    async run(message, args, cmd, client) {
        let error1 = new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.text).setTitle("No song specified");
        if (!args.length) return message.channel.send({ embeds: [error1] }); // Handles empty search queries

        let embed = new Discord.MessageEmbed().setColor(ee.color).setFooter(`Requested by ${message.author.tag}`, message.author.displayAvatarURL()); 
